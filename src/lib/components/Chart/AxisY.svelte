<!--
 ...
 <script>
  ...
  /** @type {String} [title=''] - Optional string value to add title for axis. */
  export let title = '';

  /** @type {Number} [titleGutter=0] - The amount of whitespace between the axis title and the chart drawing area (the xRange min). */
  export let titleGutter = 0;
...
<g class="axis y-axis">
  ...
  {/each}
  {#if title !== ''}
    <g class="axis-title" transform="translate({$xRange[0] + x1 - titleGutter},{$height/2}),rotate(-90)">
      <text
        text-anchor='middle'
      >{title}</text>
    </g>
